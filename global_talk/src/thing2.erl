...
%% API
-export([start/0,start/3,stop/0, send_hello/0, ask1/0]).
...
ask1() ->
    case global:whereis_name(?MODULE) of
        undefined ->
            {error, global_not_found};
        PID ->
            Value = gen_server:call(PID, ask1),
            {ok, Value}
    end.
...
handle_call(hello, _From, State) ->
        {reply, "Howdy Pardner", State};
handle_call(ask1, _From, State) ->
        {reply, thing1:reply1(), State};
